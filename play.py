    ...
    for y in range(4):
        ...
        for x in range(4):
            if(bricks_next[x][y] != 0):
                bricks_next_object[x][y].rect[0] = posX
                bricks_next_object[x][y].rect[1] = posY
...
while running:
    ...
    for event in pygame.event.get():
        ...
        showFont("最大連線數", 588, 190, color_gray)
        showFont( str(int(lines_number_max)), 588, 220, color_gray)
 
        showFont("本局連線數", 588, 260, color_gray)
        showFont( str(int(lines_number)), 588, 290, color_gray)
        # 顯示FPS.
    # 除錯訊息.
    if(debug_message):    
        showFont("FPS:" + str(clock.get_fps()), 6, 0, color_gray_green)    
